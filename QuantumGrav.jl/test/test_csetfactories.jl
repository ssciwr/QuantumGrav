...
@testitem "test_Csetfactory_works" tags=[:csetfactories] setup = [importModules, config] begin
	...
	@test csetfactory.npoint_distribution isa Distribution.DiscreteUniform
	@test params(csetfactory.npoint_distribution) ==
		  tuple(cfg["csetsize_distr_args"]...)
	@test csetfactory.config == cfg
	for key in ["random",
		"complex_topology",
		"merged",
		"merged_ambiguous",
		...
		"destroyed",
		"destroyed_ambiguous"]
		@test key in csetfactory
	end
end

@testitem "test_Csetfactory_broken_config" tags=[:csetfactories] setup = [importModules, config] begin

    config["polynomial"] = nothing

    @test_throws CsetFactory(config)
end
