...
@testitem "test_insert_cset" tags = [:csetmerging] setup = [TestsCSetMerging] begin
    cset1, _ = QuantumGrav.sample_bitarray_causet_by_connectivity(100, 0.4, 10, rng; abs_tol = 0.01)
    cset2, _ = QuantumGrav.sample_bitarray_causet_by_connectivity(30, 0.5, 10, rng2; abs_tol = 0.01)

    cset_inserted = QuantumGrav.insert_cset(cset1, cset2, 0.1; rng = rng)
...
@testitem "test_insert_cset_throws" tags = [:csetmergingthrows] setup = [TestsCSetMerging] begin
    cset1, _ = QuantumGrav.sample_bitarray_causet_by_connectivity(50, 0.4, 10, rng; abs_tol = 0.01)
    cset2, _ = QuantumGrav.sample_bitarray_causet_by_connectivity(20, 0.5, 10, rng2; abs_tol = 0.01)
    ...
    @test_throws ArgumentError QuantumGrav.insert_cset(cset1, cset2, 0.2; rng = rng, position = -1)
    @test_throws ArgumentError QuantumGrav.insert_cset(cset1, cset2, 0.2; rng = rng, position = 51)
end

@testitem "test_insert_KR_into_manifoldlike" tags = [:csetmerging] setup = [TestsCSetMerging] begin
    n2_rel = 0.05
    total_size = 200 + round(Int, 200 * n2_rel)
    cset, flag, coords = QuantumGrav.insert_KR_into_manifoldlike(200, 10, 1.5, 0.2; rng = rng, n2_rel = n2_rel)
...
@testitem "test_insert_KR_into_manifoldlike_throws" tags = [:csetmergingthrows] setup = [TestsCSetMerging] begin
    @test_throws ArgumentError QuantumGrav.insert_KR_into_manifoldlike(100, 10, 1.0, -0.1; rng = rng)
    @test_throws ArgumentError QuantumGrav.insert_KR_into_manifoldlike(100, 10, 1.0, 1.1; rng = rng)
    @test_throws ArgumentError QuantumGrav.insert_KR_into_manifoldlike(100, 10, 1.0, 0.5; rng = rng, position = -1)
    @test_throws ArgumentError QuantumGrav.insert_KR_into_manifoldlike(100, 10, 1.0, 0.5; rng = rng, position = 101)
    @test_throws ArgumentError QuantumGrav.insert_KR_into_manifoldlike(100, 10, 1.0, 0.5; rng = rng, n2_rel = 0.)
end