...
@testitem "test_make_simple_cset" tags = [:csetgeneration] setup = [setupTests] begin
    type = Float32
    cset, sprinkling = QuantumGrav.make_simple_cset("Minkowski", "CausalDiamond", 100, 2,
                                                    rng; type=type)
    ...
    cset, sprinkling = QuantumGrav.make_simple_cset("Random", "BoxBoundary", 100, 3, rng;
                                                    type=type)
    ...
    @test_throws ArgumentError QuantumGrav.make_simple_cset("Minkowski",
                                                            "CausalDiamond",
                                                            0,
                                                            4,
                                                            rng;
...
@testitem "test_make_manifold_cset" tags = [:chebyshev_causets] setup = [setupTests] begin
    ...
    cset, sprinkling, chebyshev_coefs = QuantumGrav.make_manifold_cset(npoints, rng, order,
                                                                       r; d=2, type=Float32)
...
@testitem "test_make_manifold_cset_positivity_of_squared_polynomial" tags = [:chebyshev_causets,
                                                                             :positivity] setup = [setupTests] begin
    r = 1.0 + rand(rng, r_distribution)
    npoints = rand(rng, npoint_distribution)
    order = rand(rng, order_distribution)

    chebyshev_coefs = [r^(-i - j) * randn(rng) for i in 1:order, j in 1:order]
    ...
    for _ in 1:400
        x = 2rand(rng) - 1
        y = 2rand(rng) - 1
        val = sum(squared[i, j] * x^(i - 1) * y^(j - 1)
                  for
                  i in 1:size(squared, 1), j in 1:size(squared, 2))
        @test val ≥ -1e-10  # allow minor negative values due to roundoff
    end
end

@testitem "test_make_manifold_cset_squared_polynomial_symmetry_from_symmetric_chebyshev" tags = [:chebyshev_causets,
                                                                                                 :symmetry] setup = [setupTests] begin
    r = 1.0 + rand(rng, r_distribution)
    npoints = rand(rng, npoint_distribution)
    order = rand(rng, order_distribution)
...
@testitem "make_general_cset_works" tags = [:csetgeneration] setup = [setupTests] begin
    ...
    cset, sprinkling, chebyshev_coefs, manifold_like = QuantumGrav.make_general_cset(rng,
                                                                                     npoints,
                                                                                     order,
                                                                                     r, d,
                                                                                     Float32)
