...
function build_distr(cfg::AbstractDict, name::String)::Distributions.Distribution
    ...
    if !(kwargs isa AbstractDict{Symbol,Any})
...
"""
	rcm::RandomCsetMaker(n::Int64, rng::Random.AbstractRNG; config::Union{AbstractDict, Nothing} = nothing)
...
"""
	...
	Create a new `grid` causal set maker object from the config dictionary for polynomial spacetimes.
"""
function GridCsetMakerPolynomial(config::AbstractDict)
...
function (gcm::GridCsetMakerPolynomial)(
    ...
    if isnothing(config)
        throw(
            ArgumentError(
                "Error, GridCsetMaker call operator **must** get a config keyword argument supplying the config file",
            ),
        )
    end
...
"""
	cf::CsetFactory(csetname::String, n::Int64, rng::Random.AbstractRNG; config::Union{AbstractDict, Nothing} = nothing)
...
function (cf::CsetFactory)(
    ...
    rng::Random.AbstractRNG;
    config::Union{AbstractDict,Nothing} = nothing,
)::Tuple{CausalSets.BitArrayCauset,Vararg{Any}}
    cset_and_additional_quantities = cf.cset_makers[csetname](n, rng; config = config)

    if cset_and_additional_quantities isa Tuple
        return cset_and_additional_quantities
    else
        return cset_and_additional_quantities, nothing
    end
end
