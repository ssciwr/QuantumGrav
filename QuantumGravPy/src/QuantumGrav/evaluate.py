...
def make_loss_statistics(
    ...
    Args:
        loss_data (pd.DataFrame): DataFrame to store loss statistics.
    ...
    Returns:
        pd.DataFrame: Updated DataFrame with loss statistics.
    """
...
def train_epoch(
    ...
    Args:
        ...
        loss_data (pd.DataFrame): DataFrame to store loss statistics
        process_loss (Callable[ [torch.Tensor  |  float, Collection[float], Collection[torch.tensor]], pd.DataFrame, ], optional): Function to process the loss data. Defaults to None.
    ...
    for batch in tqdm(data_loader, desc="Training epoch"):
        ...
        outputs = evaluate_batch(model, data, device, apply_model)
...
def test_epoch(
    ...
    Args:
        ...
        loss_data (pd.DataFrame): DataFrame to store loss statistics.
        process_loss (Callable[ [torch.Tensor  |  float, Collection[float], Collection[torch.tensor]], pd.DataFrame, ], optional): Function to process the loss data. Defaults to make_loss_statistics.
        device (torch.device, optional): The device to run the evaluation on. Defaults to torch.device("cpu").
        apply_model (Callable[[torch.nn.Module, torch_geometric.data.Data], Any], optional): A function to apply the model to the data. Defaults to None.
    """
    model.eval()
    ...
    epoch_loss_data = []
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Testing epoch"):
            data = batch.to(device)
            outputs = evaluate_batch(model, data, device, apply_model)
            loss = criterion(outputs, data)
            epoch_loss_data.append(loss)
